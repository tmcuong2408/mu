...
<p>
  ...
  print("X=", X.to_set())
  # 1. Phép trừ trong Không gian Đơn điểm (pw) -> Triệt tiêu rác số học[cite: 1]
  res_pw = Arithmetic.sub(X, X, space="pw")
  print(f"(X - X)_pw = {sorted(list(res_pw.to_set()))}")

  # 2. Phép trừ trong Không gian Minkowski (m) -> Phình khoảng độc lập[cite: 1]
  res_m = Arithmetic.sub(X, X, space="m")
  print(f"(X - X)_m  = {sorted(list(res_m.to_set()))}")

  # 3. Phép nhân trong Không gian Mở rộng (epw / em)[cite: 1]
  res_epw = Arithmetic.mul(X, 10, space="epw")
  print(f"(X * 10)_epw = {sorted(list(res_epw.to_set()))}")

  res_em = Arithmetic.mul(10, X, space="em")
  print(f"(10*X)_em  = {sorted(list(res_em.to_set()))}")
  ...
  print(f"(0.5 * {{2, 3, 4}})_em = {sorted(list(res_em_half.to_set()))}")

  # 5. Sử dụng hàm pw, epw, m, em với đầu vào lambda:
  # Ví dụ 3.2: f(X) = (X^2 + 5X + 6)_1
  f_pw = pw(lambda x: x**2 + 5*x + 6, X)
  print(f"pw(lambda x: x^2 + 5x + 6, X) = {sorted(list(f_pw.to_set()))}")

  # Triệt tiêu rác số học: f(x) = x - x
  zero_pw = pw(lambda x: x - x, X)
  print(f"pw(lambda x: x - x, X) = {sorted(list(zero_pw.to_set()))}")
  ...
  print(f"m(lambda a, b: a + b, A, B) = {sorted(list(res_m_fn.to_set()))}")
